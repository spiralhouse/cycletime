...
allowed-tools: mcp__linear-server__get_issue, mcp__linear-server__list_issues, Read, Write, Glob, Bash
model: sonnet
---
...
IF Linear issue ID detected:
  ...
  (e.g., "/tmp/spi-1155-handoff-summary.md")
ELSE:
  timestamp = current ISO 8601 timestamp
  filename = "/tmp/handoff-{timestamp}.md"
  (e.g., "/tmp/handoff-2025-11-19T143045Z.md")
```
...
**Test Baseline**: {baseline count if established, or "Not captured"}

**Latest Test Results**:
- Unit tests: {count} passing / {count} failing (if run)
- Integration tests: {count} passing / {count} failing (if run)
- Full suite: `./gradlew test` status
...
**Example Output**:
```
✓ Handoff summary written to: /tmp/spi-1155-handoff-summary.md

Captured:
- Linear issue: SPI-1155 (Session continuity system implementation)
- Git state: 2 new files on branch feat/spi-1154-*
- Todo list: 9 tasks (0 completed, 1 in progress, 8 pending)
- 3 key decisions documented
- Next step: Complete /write-handoff command implementation

To resume this work in a new session:
  /read-handoff /tmp/spi-1155-handoff-summary.md
```
...
This command pairs with `/read-handoff` (companion command):
