...
3. Set the correct status for each task:
   - Completed tasks: Use "completed" status
   - In-progress task: Use "in_progress" status with active forms if present
   - Pending tasks: Use "pending" status
...
**Baseline**: {baseline count if captured}
**Latest Results**: {test status from handoff}

**Pending Tests**:
{List from handoff}

**Next Test Action**: {What testing step to continue from}
...
# End of session
/write-handoff
# Output: /tmp/spi-1155-handoff-summary.md
