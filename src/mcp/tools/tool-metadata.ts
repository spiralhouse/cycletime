...
import type { ToolMetadata, ToolParameterSchema, ToolCapability } from './tool-interface.js';
...
export class ToolMetadataManager {
    ...
    if (!metadata.capabilities) {
      ...
      for (const capability of metadata.capabilities) {
        if (!this.isValidCapability(capability)) {
          errors.push(
            `invalid capability: ${capability}. Must be one of: ${ToolMetadataManager.VALID_CAPABILITIES.join(', ')}`
          );
        }
      }
    }
   ...
   */
  compareMetadata(original: ToolMetadata, updated: ToolMetadata): MetadataComparisonResult {
    const changedFields: string[] = [];
    const changes: Record<string, { from: any; to: any }> = {};

    // Compare primitive fields
    const fieldsToCompare: (keyof ToolMetadata)[] = ['name', 'description', 'version', 'category'];
  ...
  createSummary(metadata: ToolMetadata): string {
    ...
    return (
      `${metadata.name} v${metadata.version} - ${metadata.description} ` +
