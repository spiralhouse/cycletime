...
export class SQLiteCapabilityProbe implements CapabilityProbe {
  ...
  async probeCapability(
    ...
    const sqliteProvider = provider as SQLiteProvider
    ...
    try {
      ...
      return {
        capabilityId,
        isSupported: false,
        error: this.createProbeError(capabilityId, error.message),
        probedAt: new Date(),
        metadata: {
          probeDepth: options.probeDepth || 'shallow',
          probeDuration: Date.now() - startTime,
  ...
  getProviderCapabilityInfo(capabilityId: string): {
    ...
    const capabilityInfo: Record<string, {
      implementationDetails: string
      limitations?: string[]
      performanceNotes?: string
    }> = {
      'projects.create': {
        implementationDetails: 'Native SQLite implementation with ACID transactions and foreign key constraints',
      ...
      'dependencies.graph': {
        implementationDetails: 'Optimized graph traversal with cycle detection using recursive CTEs',
        performanceNotes: 'Handles 50,000+ node graphs efficiently'
      },
      'performance.caching': {
        implementationDetails: 'Intelligent query result caching with 30-second TTL and LRU eviction',
  ...
  private async probeProjectCreation(
    ...
    try {
      const testProjectId = `probe_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
  ...
  private async probeProjectReading(
    ...
    if (options.probeDepth === 'shallow') {
      return typeof provider.getProject === 'function' && typeof provider.listProjects === 'function'
    }
  ...
  private async probeIssueListing(
    ...
    try {
      ...
      return error.message.includes('not found') || error.message.includes('RESOURCE_NOT_FOUND')
    }
  }
   ...
   */
  private async fallbackCapabilityCheck(
    capabilityId: string,
    providerInfo: any
  ): Promise<boolean> {
