...
import {
  ...
  ANALYTICS_QUERIES,
  VALIDATION_QUERIES,
  // buildIssueFilterQuery - currently unused
} from './sqlite-queries.js';

import type { Issue, IssueComment } from '../../database/models/schema-types.js';
import type {
  Project,
  EnhancedIssue,
  WorkflowState,
...
export class SQLiteOperations {
  ...
  private cacheTimeout = 30_000; // 30 seconds
   ...
   */
  async createProject(input: ProjectConfig): Promise<Project> {
    ...
    try {
      ...
      const result = await this.connectionManager.executeQuery<Project>(PROJECT_QUERIES.GET_BY_ID, [
        input.id,
      ]);
  ...
  async listProjects(filters?: { name?: string; createdAfter?: Date }): Promise<Project[]> {
    ...
    try {
      const result = await this.connectionManager.executeQuery<Project>(PROJECT_QUERIES.LIST, [
        filters?.name || null,
        filters?.name || null,
  ...
  async deleteProject(id: string): Promise<void> {
    ...
    try {
      const result = await this.connectionManager.executeQuery(PROJECT_QUERIES.DELETE, [id]);
  ...
  async projectExists(id: string): Promise<boolean> {
    try {
      const result = await this.connectionManager.executeQuery(PROJECT_QUERIES.EXISTS, [id]);
   ...
   */
  async createIssue(input: IssueConfig): Promise<EnhancedIssue> {
    const startTime = Date.now();
  ...
  async getIssue(id: string): Promise<EnhancedIssue> {
    ...
    try {
      const result = await this.connectionManager.executeQuery(ISSUE_QUERIES.GET_BY_ID, [id]);
  ...
  async listIssues(filters: IssueFilters): Promise<EnhancedIssue[]> {
    ...
    try {
      ...
      const params = [
        ...
        filters.has_estimate || null,
        filters.has_estimate || null,
        // Date filters (doubled for LIKE pattern)
        filters.created_after?.toISOString() || null,
        filters.created_after?.toISOString() || null,
        filters.created_before?.toISOString() || null,
        filters.created_before?.toISOString() || null,
        filters.updated_after?.toISOString() || null,
        filters.updated_after?.toISOString() || null,
        filters.updated_before?.toISOString() || null,
        filters.updated_before?.toISOString() || null,
      ...
      const result = await this.connectionManager.executeQuery(ISSUE_QUERIES.LIST, params);
  ...
  async deleteIssue(id: string): Promise<void> {
    ...
    try {
      const result = await this.connectionManager.executeQuery(ISSUE_QUERIES.DELETE, [id]);
  ...
  async issueExists(id: string): Promise<boolean> {
    try {
      const result = await this.connectionManager.executeQuery(ISSUE_QUERIES.EXISTS, [id]);
  ...
  async getAvailableIssues(projectId: string, assigneeId?: string): Promise<EnhancedIssue[]> {
    ...
    try {
      const result = await this.connectionManager.executeQuery(ISSUE_QUERIES.GET_AVAILABLE_ISSUES, [
        projectId,
        assigneeId || null,
        assigneeId || null,
      ]);
  ...
  async createDependency(
    ...
    try {
      ...
      const existsResult = await this.connectionManager.executeQuery(DEPENDENCY_QUERIES.EXISTS, [
        blockerId,
        blockedId,
      ]);
      ...
      const result = await this.connectionManager.executeQuery(DEPENDENCY_QUERIES.GET_BY_ID, [id]);
  ...
  async deleteDependency(id: string): Promise<void> {
    ...
    try {
      const result = await this.connectionManager.executeQuery(DEPENDENCY_QUERIES.DELETE, [id]);
  ...
  async createLabel(input: CreateLabelInput): Promise<Label> {
    ...
    try {
      ...
      const result = await this.connectionManager.executeQuery<Label>(LABEL_QUERIES.GET_BY_ID, [
        input.id,
      ]);
  ...
  async addLabelToIssue(issueId: string, labelId: string): Promise<void> {
    ...
    try {
      await this.connectionManager.executeQuery(LABEL_QUERIES.ADD_TO_ISSUE, [issueId, labelId]);
   ...
   */
  async getDependencyBottlenecks(projectId: string): Promise<
    {
      issueId: string;
      title: string;
      issueType: string;
  ...
  async validateProjectIntegrity(projectId: string): Promise<{
    ...
    try {
      ...
      const statsResult = await this.connectionManager.executeQuery(PROJECT_QUERIES.STATS, [
        projectId,
      ]);
  ...
  private async getIssueDependencies(issueId: string): Promise<Dependency[]> {
    const result = await this.connectionManager.executeQuery(DEPENDENCY_QUERIES.GET_BLOCKED, [
      issueId,
    ]);
  ...
  private async getIssueDependents(issueId: string): Promise<Dependency[]> {
    const result = await this.connectionManager.executeQuery(DEPENDENCY_QUERIES.GET_BLOCKERS, [
      issueId,
    ]);
  ...
  private async getIssueChildren(issueId: string): Promise<EnhancedIssue[]> {
    const result = await this.connectionManager.executeQuery(ISSUE_QUERIES.GET_CHILDREN, [issueId]);
  ...
  private mapEnhancedIssueRow(
    ...
    const enhancedIssue: EnhancedIssue = {
      ...
      ...(relationships?.labels &&
        relationships.labels.length > 0 && { labels: relationships.labels }),
      ...(relationships?.dependencies &&
        relationships.dependencies.length > 0 && { dependencies: relationships.dependencies }),
      ...(relationships?.dependents &&
        relationships.dependents.length > 0 && { dependents: relationships.dependents }),
      ...(relationships?.comments &&
        relationships.comments.length > 0 && { comments: relationships.comments }),
      ...(workflowState && { workflowState }),
      ...(relationships?.children &&
        relationships.children.length > 0 && { children: relationships.children }),
    };
  ...
  private updateMetrics(duration: number): void {
    ...
    this.performanceMetrics.averageQueryTime =
      (currentAverage * (queryCount - 1) + duration) / queryCount;
  }
    ...
    return {
      ...
      cacheHitRate:
        this.performanceMetrics.totalQueries > 0
          ? this.performanceMetrics.cachedQueries / this.performanceMetrics.totalQueries
          : 0,
