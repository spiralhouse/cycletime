...
import type { IssueType, IssuePriority } from '../../database/models/schema-types.js';
import type { EnhancedIssue, TaskRecommendation, DependencyGraph } from '../types.js';
...
export class TaskRecommendationEngine {
    ...
    try {
      ...
      return {
        ...
        context: {
          ...
          ...(context.recentWork && { recentWork: context.recentWork }),
      ...
      throw new Error(`Task recommendation failed: ${message}`);
    }
  }
  ...
  async getMultipleRecommendations(
    ...
    assigneeId?: string,
    context: RecommendationContext = {},
    strategies: ('priority' | 'quick_wins' | 'unblocking' | 'context_based')[] = ['priority']
  ): Promise<TaskRecommendation[]> {
  ...
  private calculateComplexityScore(issue: EnhancedIssue): number {
    ...
    const typeScore = typeScores[issue.issue_type];
    ...
    if (issue.estimate) {
      if (issue.estimate <= 3)
        score += 2; // Small tasks
      else if (issue.estimate <= 5)
        score += 1; // Medium tasks
  ...
  private calculateTimeFitScore(issue: EnhancedIssue, context: RecommendationContext): number {
    ...
    if (issue.estimate) {
      ...
      if (estimatedHours <= availableHours) {
        ...
        else if (preferredTaskSize === 'medium' && estimatedHours >= 3 && estimatedHours <= 5)
          score += 2;
   ...
   */
  private calculateBusinessValueScore(issue: EnhancedIssue, bottlenecks: any[]): number {
    let score = 5; // Baseline
  ...
  private matchesWorkPattern(issue: EnhancedIssue, recentWork: string[]): boolean {
    ...
    return recentWork.some(
      work => issueContext.includes(work.toLowerCase()) || issue.issue_type === work.toLowerCase()
    );
  }
  ...
  private getIssueTypeContext(issueType: IssueType): string {
    const contexts: Record<IssueType, string> = {
      ...
      story:
        'This story represents valuable user functionality that can be delivered incrementally.',
  ...
  private createNoTasksRecommendation(
    ...
    return {
      ...
      confidence: 1.0,
      rationale:
        'No available tasks found. All current issues are either completed, blocked by dependencies, or assigned to others.',
      alternatives: [],
      context: {
        availableIssues: 0,
        ...(context.focusArea && { focusArea: context.focusArea }),
