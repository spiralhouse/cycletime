...
export class FieldMapper {
  ...
  async mapField<TSource, TTarget>(
    ...
    try {
      ...
      if (mapping.typeConversion) {
        transformedValue = await this.applyTypeConversion(
          transformedValue,
      ...
      result.errors.push({
        ...
        message: `Field mapping failed for '${mapping.targetField}': ${errorMessage}`,
        field: mapping.targetField,
        sourceValue,
        recoverable: false,
        context: {
  ...
  private async applyIdentityMapping<TSource, TTarget>(
    sourceValue: any,
    _mapping: FieldMapping<TSource, TTarget>,
  ...
  private async applyTypeConversion<T>(
    ...
    try {
      ...
      throw new Error(
        `Type conversion failed from ${conversion.sourceType} to ${conversion.targetType}: ${errorMessage}`
      );
    }
  }
  ...
  private convertToString(value: any, conversion: TypeConversion): string {
    ...
    if (value instanceof Date)
      return conversion.options?.dateFormat
        ? this.formatDate(value, conversion.options.dateFormat)
        : value.toISOString();
  ...
  private convertToNumber(value: any, conversion: TypeConversion): number {
    ...
    if (typeof value === 'string') {
      const parsed = conversion.options?.numberBase
        ? Number.parseInt(value, conversion.options.numberBase)
  ...
  private convertToDate(value: any, _conversion: TypeConversion): Date {
  ...
  private convertToArray(value: any, _conversion: TypeConversion): any[] {
  ...
  private async validateFieldValue<T>(
    ...
    result.score =
      totalChecks > 0 ? Math.max(0, (totalChecks - result.errors.length) / totalChecks) : 1.0;
...
export function createStandardMapping<TSource, TTarget>(
  ...
  const mapping: FieldMapping<TSource, TTarget> = {
    sourceField,
    targetField,
    strategy: 'identity',
  ...
  if (options.defaultValue !== undefined) {
    mapping.defaultValue = options.defaultValue;
  }
  if (options.typeConversion !== undefined) {
    mapping.typeConversion = options.typeConversion;
  }
  if (options.validation !== undefined) {
    mapping.validation = options.validation;
  }
...
export function createComputedMapping<TSource, TTarget>(
  ...
  if (reverseTransform !== undefined) {
    mapping.reverseTransform = reverseTransform;
  }
  if (options.validation !== undefined) {
    mapping.validation = options.validation;
  }
...
export function createLookupMapping<TSource, TTarget>(
  sourceField: string,
  targetField: string,
  _lookupTable: Record<string, TTarget>,
  options: {
    required?: boolean;
    defaultValue?: TTarget;
  } = {}
