...
export class ProviderConfigValidator {
  ...
  validate(config: ProviderConfig): ValidationResult {
    ...
    try {
      ...
      return {
        ...
        errors,
        warnings,
        ...(errors.length === 0 && { sanitizedConfig }),
      };
    } catch (error) {
      errors.push({
        field: 'validation',
  ...
  validateProviderType(config: ProviderConfig, type: ProviderType): ValidationResult {
    ...
    const typedConfig = { ...config, type } as ProviderConfig;
    ...
    return {
      common: [
        ...
        },
        {
          ...
          pattern: /^[\w-]+$/,
          message:
            'Provider ID must be 1-100 characters, alphanumeric with underscores and hyphens only',
        },
        {
          field: 'name',
          type: 'required',
    ...
    if (field.includes('token') && typeof value === 'string') {
      ...
      if (value.toLowerCase().includes('test') || value.toLowerCase().includes('demo')) {
        warnings.push({
          field,
          message:
            'Token appears to be for testing - ensure production tokens are used in production',
          value,
   ...
   */
  private sanitizeConfig(config: ProviderConfig, _warnings: ValidationWarning[]): ProviderConfig {
...
export function createCustomValidator(
  _customRules?: Partial<ValidationSchema>
