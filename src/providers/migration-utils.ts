...
export class MigrationProgressTracker extends EventEmitter {
  ...
  private processingStartTime: Date = new Date()
  ...
  private getCompletedPhaseWeight(phaseWeights: Record<MigrationPhase, number>): number {
    const phaseOrder: MigrationPhase[] = [
      'initializing', 'exporting', 'validating', 'transforming', 'importing', 'verifying'
    ]
  ...
  private calculateProcessingRate(): void {
    ...
    if (elapsedSeconds > 0) {
      ...
      if (this.progress.processingRate > 0) {
        ...
        this.progress.estimatedTimeRemaining = (remainingEntities / this.progress.processingRate) * 1000
      }
    }
  }
}
...
export class StreamingExportTransform extends Transform {
  ...
  _transform(chunk: any, encoding: string, callback: Function): void {
    ...
    const shouldFlush = 
      this.chunkBuffer.length >= this.config.chunkSize ||
...
export class StreamingImportProcessor {
   ...
   */
  private async processProjectsInChunks(
    projects: any[], 
    result: ImportResult
  ): Promise<void> {
   ...
   */
  private async processWorkflowStatesInChunks(
    states: any[], 
    result: ImportResult
  ): Promise<void> {
   ...
   */
  private async processLabelsInChunks(
    labels: any[], 
    result: ImportResult
  ): Promise<void> {
   ...
   */
  private async processIssuesInChunks(
    issues: any[], 
    result: ImportResult
  ): Promise<void> {
    // Sort issues by hierarchy: epics first, then stories, then subtasks
    const hierarchyOrder = { epic: 0, story: 1, subtask: 2 }
    const sortedIssues = [...issues].sort((a, b) => 
      hierarchyOrder[a.issue_type] - hierarchyOrder[b.issue_type]
    )
   ...
   */
  private async processCommentsInChunks(
    comments: any[], 
    result: ImportResult
  ): Promise<void> {
...
export class MigrationOrchestrator {
  ...
  private async verifyMigration(
    ...
    if (exportCounts.projects !== importCounts.projects ||
  ...
  private calculateTotalEntities(exportData?: ExportData): number {
    ...
    return counts.projects + counts.issues + counts.dependencies + 
           counts.workflowStates + counts.labels + counts.comments
  }

  /**
   * Calculate migration throughput
   */
  private calculateThroughput(exportData?: ExportData, duration: number): number {
...
export class MigrationRollbackManager {
   ...
   */
  private async restoreProviderState(state: any): Promise<void> {
