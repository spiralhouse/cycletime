...
export class HealthMonitor {
   ...
   */
  private updateStatus(isHealthy: boolean, error?: ProviderError, responseTime?: number): void {
    this.currentStatus = {
...
export class DatabaseHealthMonitor extends HealthMonitor {
  constructor(
    ...
    super(connectionTestFn, {
      checkInterval: 60000, // 1 minute for databases
   ...
   */
  async performDatabaseHealthCheck(): Promise<HealthCheckResult & {
    connectionPool?: {
      active: number
      idle: number
      total: number
...
export class APIHealthMonitor extends HealthMonitor {
  constructor(
    apiHealthCheckFn: () => Promise<boolean>,
    options: HealthMonitorOptions = {}
  ) {
    super(apiHealthCheckFn, {
      checkInterval: 30000, // 30 seconds for APIs
      maxRetries: 3,
      timeoutMs: 15000, // Longer timeout for API calls
      ...options
    })
  }
