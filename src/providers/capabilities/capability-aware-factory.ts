...
function createMockCapabilityDiscoverySystem(options?: {
  enableCaching?: boolean;
}): MockCapabilityDiscoverySystem {
  return {
    registry: {
...
export class CapabilityAwareProviderFactory implements ProviderFactory {
  ...
  async getProviderTypeCapabilities(providerType: ProviderType): Promise<{
    ...
    const _registry = this.discoverySystem.registry;
    const allCapabilities = _registry.getAllCapabilities();
  ...
  async findBestProviderForCapabilities(
    ...
    for (const config of availableConfigs) {
      try {
        ...
        const score =
          requiredCapabilities.length > 0 ? supported.length / requiredCapabilities.length : 0;
   ...
   */
  private async initializeProviderCapabilities(_provider: IssueProvider): Promise<void> {
    // If provider supports capability discovery, set it up
    if (_provider.discoverCapabilities) {
      try {
        // Perform initial capability discovery with caching
        const discoveryResult = await this.discoverySystem.engine.discoverCapabilities(_provider, {
          probeDepth: 'shallow',
          skipCached: false,
          timeout: 5000,
        });
        ...
        console.warn(
          `Capability discovery failed for provider ${_provider.getProviderInfo().id}:`,
          error
        );
      }
    }
  ...
  private async validateProviderCapabilities(
    ...
    if (_provider.validateCapabilities) {
      return await _provider.validateCapabilities(requiredCapabilities);
    }
    ...
    try {
      ...
      for (const [capabilityId, probeResult] of discoveryResult.capabilities) {
        if (probeResult.isSupported) {
          ...
          if (probeResult.metadata?.limitations?.length) {
            warnings.push(
              `${capabilityId} has limitations: ${probeResult.metadata.limitations.join(', ')}`
            );
          }
        } else {
      ...
      warnings.push(
        `Capability discovery failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
...
export class CapabilityAwareOperationDispatcher {
  ...
  async executeWithCapabilityCheck<T>(
    ...
    if (!validationResult.canExecute) {
      const error: ProviderError = {
        ...
        providerId: _provider.getProviderInfo().id,
        providerType: _provider.getProviderInfo().type,
        retryable: false,
        context: {
        ...
        ...(validationResult.alternatives && { userActions: validationResult.alternatives }),
      };
  ...
  async getOperationAlternatives(
    ...
    for (const capabilityId of unsupportedCapabilities) {
      const capability = _registry.getCapability(capabilityId);
      ...
      if (capability?.alternatives) {
        for (const altId of capability.alternatives) {
          const altCapability = _registry.getCapability(altId);
      ...
      if (_provider.getCapabilityInfo) {
        try {
          const capabilityInfo = await _provider.getCapabilityInfo(capabilityId);
  ...
  private async validateOperationCapabilities(
    ...
    // Perform validation
    if (_provider.validateCapabilities) {
      validationResult = await _provider.validateCapabilities(requiredCapabilities);
    } else {
      // Fallback validation by checking individual capabilities
      const unsupported: string[] = [];

      for (const capabilityId of requiredCapabilities) {
        if (_provider.supportsCapability) {
          const isSupported = await _provider.supportsCapability(capabilityId);
