    ...
    describe("ToolRegistry") {
        ...
        describe("tool definition and interface") {
            ...
            it("should support synchronous tool handlers") {
                ...
                // Handler is now a ToolHandler.Sync, need to extract the actual handler
                val handler = (syncTool.handler as ToolHandler.Sync).handler
                val result = handler(JsonObject(emptyMap()))
            ...
            it("should support asynchronous tool handlers") {
                ...
                runBlocking {
                    val handler = (asyncTool.handler as ToolHandler.Async).handler
                    val result = handler(JsonObject(emptyMap()))
                    result.isSuccess shouldBe true
