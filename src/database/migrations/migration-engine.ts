...
export class MigrationEngine {
  ...
  constructor(config: MigrationEngineConfig, hooks: MigrationHooks = {}, logger?: MigrationLogger) {
    this.config = config;
    this.hooks = hooks;
  ...
  async createMigrationPlan(
    ...
    const planContext = this.createMigrationContext(
      null,
      direction,
      'normal',
      currentVersion,
  ...
  private buildMigrationPlan(
    ...
    if (direction === 'up') {
      ...
      planMigrations = allMigrations.filter(
        migration =>
   ...
   */
  async executePlan(plan: MigrationPlan, mode: MigrationMode = 'normal'): Promise<MigrationResult> {
    if (!plan.validation_results.is_valid) {
      throw new Error(
        `Cannot execute invalid migration plan: ${plan.validation_results.errors.map(e => e.message).join(', ')}`
      );
    }
