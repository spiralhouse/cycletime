...
import type { SemanticVersion, SchemaComparison } from './migration-types.js';
...
export function parseSemanticVersion(versionString: string): SemanticVersion {
  ...
  if (!match) {
    throw new Error(
      `Invalid semantic version format: ${versionString}. Expected: MAJOR.MINOR.PATCH[-prerelease][+build]`
    );
  }
  ...
  if (!major || !minor || !patch) {
    throw new Error(
      `Invalid semantic version format: ${versionString}. Missing major, minor, or patch version.`
    );
  }
...
export function getNextVersion(
  ...
  if (!prerelease) {
    delete nextVersion.prerelease;
    delete nextVersion.build;
  } else {
...
export function parseVersionRange(rangeString: string): VersionRange {
  ...
  return {
    operator: finalOperator as VersionRange['operator'],
    version,
  };
}
...
export function analyzeSchemaChanges(
  ...
  throw new Error(
    `Cannot analyze changes from ${formatSemanticVersion(fromVersion)} to ${formatSemanticVersion(toVersion)}: target version is older`
  );
}
...
export function determineVersionBump(
  ...
  if (
    ...
    schemaComparison.modified_tables.some(
      table =>
        table.removed_columns.length > 0 ||
        table.removed_constraints.length > 0 ||
        table.modified_columns.some(
          col =>
  ...
  if (
    ...
    schemaComparison.modified_tables.some(
      table => table.added_columns.length > 0 || table.added_constraints.length > 0
    )
  ) {
    return 'minor';
  }
