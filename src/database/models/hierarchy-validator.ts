...
import type { Issue, IssueType } from './schema-types.js';
...
export function validateIssueHierarchy(
  ...
  if (issue.parent_id && !parentIssue) {
    warnings.push({
      ...
      message:
        'Parent issue not found in provided issue list. This may be valid if parent exists in database.',
      field: 'parent_id',
  ...
  if (parentIssue) {
    ...
    if (depth > 3) {
      errors.push({
        ...
        message:
          'Maximum hierarchy depth of 3 levels exceeded. Current structure: Epic → Story → Subtask',
        field: 'parent_id',
...
export function validateIssueHierarchyBatch(issues: Issue[]): ValidationResult {
  ...
  const orphanedSubtasks = issues.filter(
    issue =>
