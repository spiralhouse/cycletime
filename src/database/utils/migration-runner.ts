...
import { DefaultMigrationEngineFactory } from '../migrations/migration-engine.js';
import { MigrationValidator } from '../migrations/migration-validator.js';
import { SchemaVersioning, parseSemanticVersion } from '../migrations/schema-versioning.js';

import type { MigrationEngine } from '../migrations/migration-engine.js';
import type {
...
class MigrationRunner {
   ...
   */
  async create(name: string, _options: MigrationRunnerOptions = {}): Promise<void> {
    const directory = _options.directory || this.config.migration_directories[0];
    ...
    this.generateMigrationTemplate(migrationId, name, nextVersion);
  ...
  private async getLatestVersion(): Promise<SemanticVersion> {
    ...
    return sortedMigrations[0]?.version || parseSemanticVersion('1.0.0');
  }
  ...
  private loadMigrationFromFile(filePath: string): Migration | null {
    try {
      return filePath.endsWith('.sql')
        ? this.loadSQLMigration(filePath)
        : this.loadJSMigration(filePath);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);

      console.warn(`⚠️  Failed to load migration from ${filePath}: ${errorMessage}`);
  ...
  private loadJSMigration(_filePath: string): Migration {
...
async function main() {
  try {
    ...
    switch (cli.command) {
      ...
      case 'up':
        const targetVersion = cli.options.target
          ? parseSemanticVersion(cli.options.target)
          : undefined;
      ...
      case 'down':
        ...
        const rollbackTarget = parseSemanticVersion(cli.options.target as string);
...
function parseCLI(): CLIConfig {
  ...
  return { command: command || '', options, args: remainingArgs };
}
...
function loadConfig(options: MigrationRunnerOptions): MigrationEngineConfig {
  ...
  return {
    ...
    migration_directories: [options.directory || './src/database/migrations'],
