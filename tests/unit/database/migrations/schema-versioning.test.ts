...
describe('Schema Versioning System', () => {
  describe('parseSemanticVersion', () => {
    ...
    it('should throw error for invalid versions', () => {
      const invalidVersions = ['1.2', '1.2.3.4', 'invalid', '', '1.2.x', 'v1.2.3'];
  ...
  describe('formatSemanticVersion', () => {
    it('should format semantic versions correctly', () => {
      const testCases = [
        ...
        {
          input: { major: 1, minor: 2, patch: 3, prerelease: 'alpha', build: 'build' },
          expected: '1.2.3-alpha+build',
        },
      ];
  ...
  describe('validateVersionProgression', () => {
    it('should validate version progression', () => {
      ...
      expect(validateVersionProgression(current, parseSemanticVersion('1.2.2')).isValid).toBe(
        false
      );
      expect(validateVersionProgression(current, parseSemanticVersion('1.1.0')).isValid).toBe(
        false
      );
      expect(validateVersionProgression(current, parseSemanticVersion('0.9.0')).isValid).toBe(
        false
      );
    });
  });
