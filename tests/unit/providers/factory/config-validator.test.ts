...
describe('ProviderConfigValidator', () => {
  ...
  describe('Common Field Validation', () => {
    ...
    it('should validate provider type against allowed values', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors.some(e => e.field === 'type' && e.message.includes('sqlite, linear, github, jira'))).toBe(true)
    })
    ...
    it('should accept valid provider ID formats', () => {
      const validIds = ['simple-id', 'id_with_underscores', 'id-with-hyphens', 'id123', 'ID-UPPER-CASE']
  ...
  describe('SQLite Provider Validation', () => {
    ...
    it('should require database path', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors.some(e => e.field === 'databasePath' && e.rule === 'required')).toBe(true)
    })
  ...
  describe('Cross-field Validation', () => {
    it('should warn when ID and name are identical', () => {
      ...
      expect(result.isValid).toBe(true)
      expect(result.warnings.some(w => w.field === 'id' && w.message.includes('identical'))).toBe(true)
    })
