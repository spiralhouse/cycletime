...
describe('SimpleProviderFactory', () => {
  ...
  describe('Configuration Validation', () => {
    ...
    it('should reject missing type', async () => {
      ...
      await expect(factory.createProvider(invalidConfig)).rejects.toThrow('Provider type is required')
    })
    ...
    it('should reject missing ID', async () => {
      ...
      await expect(factory.createProvider(invalidConfig)).rejects.toThrow('Provider ID is required')
    })
    ...
    it('should reject missing name', async () => {
      ...
      await expect(factory.createProvider(invalidConfig)).rejects.toThrow('Provider name is required')
    })
    ...
    it('should validate SQLite-specific requirements', async () => {
      ...
      await expect(factory.createProvider(invalidConfig)).rejects.toThrow('SQLite provider requires databasePath')
    })
    ...
    it('should validate Linear-specific requirements', async () => {
      ...
      await expect(factory.createProvider(missingTokenConfig)).rejects.toThrow('Linear provider requires apiToken')
      await expect(factory.createProvider(missingTeamConfig)).rejects.toThrow('Linear provider requires teamId')
    })
    ...
    it('should validate GitHub-specific requirements', async () => {
      ...
      await expect(factory.createProvider(missingTokenConfig)).rejects.toThrow('GitHub provider requires apiToken')
      await expect(factory.createProvider(missingOwnerRepoConfig)).rejects.toThrow('GitHub provider requires owner and repo')
    })
    ...
    it('should validate Jira-specific requirements', async () => {
      ...
      for (const field of requiredFields) {
        ...
        await expect(factory.createProvider(invalidConfig)).rejects.toThrow(`Jira provider requires ${field}`)
      }
    })
  })
  ...
  describe('Provider Registry Integration', () => {
    ...
    it('should prevent duplicate provider IDs', async () => {
      ...
      await expect(factory.createProvider(validSQLiteConfig)).rejects.toThrow('Provider with ID \'test-sqlite\' already exists')
    })
  })
  ...
  describe('Error Handling', () => {
    it('should provide detailed error information', async () => {
      try {
        ...
        expect(error.message).toContain('Provider type is required')
