...
describe('EnhancedProviderFactory', () => {
  ...
  describe('Registry Integration', () => {
    ...
    it('should list registered providers', async () => {
      ...
      expect(providers.map(p => p.id)).toContain('test-linear')
    })
    ...
    it('should provide registry statistics', async () => {
      ...
      expect(stats.registry.totalProviders).toBe(2)
      expect(stats.registry.providersByType.sqlite).toBe(1)
      expect(stats.registry.providersByType.linear).toBe(1)
...
describe('Provider Factory Integration', () => {
  ...
  it('should work with different provider types simultaneously', async () => {
    ...
    const providers = await Promise.all([
      factory.createProvider(mockSQLiteConfig),
      factory.createProvider(mockLinearConfig),
    ...
    const stats = factory.getFactoryStatistics()
    expect(stats.registry.totalProviders).toBe(3)
    expect(stats.registry.healthyProviders).toBe(3)
  ...
  it('should provide comprehensive factory statistics', async () => {
    ...
    await factory.createProviders([
      mockSQLiteConfig,
      mockLinearConfig,
      { ...mockGitHubConfig, id: 'github-1', name: 'GitHub 1' },
      { ...mockGitHubConfig, id: 'github-2', name: 'GitHub 2' }
    ])

    const stats = factory.getFactoryStatistics()
    
    expect(stats.registry.totalProviders).toBe(4)
    expect(stats.registry.providersByType.sqlite).toBe(1)
    expect(stats.registry.providersByType.linear).toBe(1)
    expect(stats.registry.providersByType.github).toBe(2)
    expect(stats.instantiator.managedInstances).toBe(4)
    expect(stats.instantiator.healthyInstances).toBe(4)
  })
})