...
class MockEntityTransformer<TProvider, TUnified> implements EntityTransformer<TProvider, TUnified> {
  ...
  async transform(source: TProvider, context: TransformationContext): Promise<TransformationResult<TUnified>> {
  ...
  async reverseTransform(target: TUnified, context: TransformationContext): Promise<TransformationResult<TProvider>> {
  ...
  async transformBatch(sources: TProvider[], context: TransformationContext): Promise<BatchTransformationResult<TUnified>> {
  ...
  async reverseTransformBatch(targets: TUnified[], context: TransformationContext): Promise<BatchTransformationResult<TProvider>> {
    const startTime = new Date()
...
describe('JCVDTransformationEngine', () => {
  ...
  describe('Transformation Schema', () => {
    ...
    it('should retrieve transformation schema for provider pair', async () => {
      const schema = await transformationEngine.getTransformationSchema('linear', 'sqlite', 'issue')
    ...
    it('should cache transformation schemas', async () => {
      // First call
      const schema1 = await transformationEngine.getTransformationSchema('linear', 'sqlite', 'issue')
      // Second call should use cache
      const schema2 = await transformationEngine.getTransformationSchema('linear', 'sqlite', 'issue')
    ...
    it('should track failed transformations in statistics', async () => {
      ...
      vi.spyOn(failingTransformer, 'getEntityTransformer').mockImplementation(() => {
        ...
        vi.spyOn(transformer, 'transformBatch').mockResolvedValue({
          success: false,
          successful: [],
          failed: [{ sourceData: {}, errors: [{ code: 'TRANSFORMATION_ERROR', message: 'Mock error', recoverable: false }] }],
          warnings: [],
          metrics: {
            startTime: new Date(),
            endTime: new Date(),
  ...
  describe('Performance Targets', () => {
    ...
    it('should meet performance target of 1000+ issues/second', async () => {
      ...
      // Performance target: 1000+ issues per second
      expect(issuesPerSecond).toBeGreaterThan(1000)
    ...
    it('should maintain performance with complex nested data', async () => {
      const complexIssues = Array.from({ length: 500 }, (_, i) => ({
        id: `issue_${i}`,
