...
describe('CapabilityDiscoveryEngine', () => {
  ...
  it('should throw error when no probe is registered', async () => {
    await expect(
      engine.discoverCapabilities(mockProvider as IssueProvider)
    ).rejects.toThrow('No capability probe registered')
  })
  ...
  it('should handle provider unavailability', async () => {
    ...
    const result = await engine.discoverCapabilities(
      unavailableProvider as IssueProvider,
      { targetCapabilities: ['projects.create'] }
    )
  ...
  it('should calculate compatibility scores between providers', async () => {
    ...
    const mockProbe = {
      async probeCapability(provider: IssueProvider, capabilityId: string) {
        ...
        const isSupported = providerInfo.type === 'sqlite' || 
...
describe('Capability Discovery Integration', () => {
  ...
  it('should handle mixed success and failure scenarios', async () => {
    ...
    const supportedCount = Array.from(result.capabilities.values())
      .filter(probe => probe.isSupported).length
    
    const unsupportedCount = Array.from(result.capabilities.values())
      .filter(probe => !probe.isSupported).length
