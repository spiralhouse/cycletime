...
function createMockProvider(providerId: string, providerType: 'sqlite' | 'linear' | 'github' | 'jira'): IssueProvider {
  return {
  ...
  for (const [capabilityId, isSupported] of Object.entries(capabilitySupport)) {
    capabilities.set(capabilityId, {
      ...
      metadata: isSupported ? {
        ...
        limitations: providerInfo.type === 'github' && capabilityId.includes('workflow') ? 
          ['Limited workflow states'] : undefined
      } : {
...
describe('ProviderComparisonEngine', () => {
  ...
  it('should compare SQLite and Linear providers', async () => {
    ...
    const sqliteMatrix = await matrixGenerator.generateFeatureMatrix(sqliteProvider, sqliteDiscovery)
    const linearMatrix = await matrixGenerator.generateFeatureMatrix(linearProvider, linearDiscovery)
    ...
    const compatibleCount = Array.from(comparison.capabilityComparison.values())
      .filter(comp => comp.compatibility === 'compatible').length
    const incompatibleCount = Array.from(comparison.capabilityComparison.values())
      .filter(comp => comp.compatibility === 'incompatible').length
  ...
  it('should assess migration feasibility', async () => {
    ...
    const sourceMatrix = await matrixGenerator.generateFeatureMatrix(sourceProvider, sourceDiscovery)
    const targetMatrix = await matrixGenerator.generateFeatureMatrix(targetProvider, targetDiscovery)
