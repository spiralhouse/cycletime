...
export class PerformanceTracker {
  ...
  finalizeBenchmark(): PerformanceBenchmark {
    const testMetrics = Array.from(this.metrics.values()).filter(m =>
      m.name.startsWith(`${this.currentSuite}:${this.currentTest}:`)
    );
...
export class DatabasePerformanceAnalyzer {
  private queryMetrics: { sql: string; duration: number }[] = [];
...
export class LoadTester {
  async runLoadTest<T>(
    ...
    const errors: { operation: string; error: string; timestamp: number }[] = [];
    const startTime = performance.now();
    ...
    for (let user = 0; user < config.concurrentUsers; user++) {
      const userPromise = this.simulateUser(user, config, operationFactory, results, errors);
  ...
  private async simulateUser<T>(
    ...
    config: LoadTestConfig,
    operationFactory: () => Promise<T>,
    results: { success: boolean; duration: number; error?: string }[],
    errors: { operation: string; error: string; timestamp: number }[]
  ): Promise<void> {
  ...
  async generateReport(): Promise<PerformanceReport> {
    ...
    for (const benchmark of benchmarks) {
      for (const metric of benchmark.metrics) {
        ...
        if (
          metric.name.includes('migration') &&
          metric.duration > thresholds.migrationPerformance
        ) {
          violations.push({
            type: 'performance',
...
${violations
  .map(
    v => `- **${v.type.toUpperCase()}:** ${v.description} (${v.actual} vs ${v.expected} expected)`
  )
  .join('\n')}
...
`;
  ...
  logPerformanceSummary(report: PerformanceReport): void {
    ...
    console.log(
      `Slowest Test: ${report.summary.slowestTest.name} (${report.summary.slowestTest.duration.toFixed(2)}ms)`
    );
    console.log(
      `Fastest Test: ${report.summary.fastestTest.name} (${report.summary.fastestTest.duration.toFixed(2)}ms)`
    );
